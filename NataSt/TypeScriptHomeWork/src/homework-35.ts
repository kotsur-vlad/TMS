...
const clientsWithEducation = (clients: Client[]): Client[] => clients.filter((client: Client): boolean => client.hasEducation === true);
console.log(clientsWithEducation(clients));


// 4. Создать функцию, которая бы принимала массив пользователей и отфильтровывала пользователей на наличие животных

const clientsWithPets = (clients: Client[]): Client[] => clients.filter((client: Client): string[] | undefined => client.animals);
console.log(clientsWithPets(clients));


// 5. Создать функцию, которая бы принимала массив пользователей и отдавала бы  строку с названиями марок автомобилей через запятую

const getCarsNames = (clients: Client[]) => {
